...
pub fn gol_three(mut curr: Vec<u8>, width: usize, iterations: usize) -> Vec<u8> {
    let area = curr.len();
    ...
    for _ in 0..iterations {
        // Capture raw pointer addresses as usize (Sync) and reconstruct pointers inside the closure
        let curr_addr = padded_curr.as_ptr() as usize;
        let next_addr = padded_next.as_mut_ptr() as usize;

        // Parallel simulation over each interior row
        (1..=height).into_par_iter().for_each(move |y| {
            // Recreate raw pointers within the thread
            ...
            for x in 1..=width {
                ...
                unsafe {
                    ...
                    // Write the next state via raw pointer
